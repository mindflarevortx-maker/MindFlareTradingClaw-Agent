...
const DomClaw = (() => {
  ...
  // ── 15. getWebSocket ───────────────────────────────────────────────────
  /** Get an active WebSocket connection to market-qx.trade for sending messages.
   *  Uses the tracked sockets from page-hook.js (__MFC_WS_SOCKETS). */
  function getWebSocket() {
    try {
      // Access tracked sockets from page-hook.js (page context)
      // Content scripts can't access page variables directly, but we can
      // use the MF.state.wsUrl to know which WS is active
      if (window.__MFC_WS_SOCKETS && window.__MFC_WS_SOCKETS.length) {
        for (const ws of window.__MFC_WS_SOCKETS) {
          if (ws.readyState === WebSocket.OPEN && ws.url &&
              (ws.url.includes('market-qx') || ws.url.includes('quotex') ||
               ws.url.includes('qxbroker') || ws.url.includes('socket.io'))) {
            return ws;
          }
        }
      }
  ...
  function getBalance() {
    try {
      ...
      const selectors = [
        '[class*="balance" i]', '[class*="account-balance" i]',
        '[data-testid="balance"]', '[class*="wallet" i]',
        '[class*="funds" i]',
      ];
      for (const sel of selectors) {
        try {
